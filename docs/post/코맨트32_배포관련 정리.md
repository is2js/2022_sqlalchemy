...
2. **`alembic init alembic`은 `DIRECTORY alembic`을 기준으로 발생한다.**
...
3. **local에서 `기존 alembic 폴더 삭제` 하고 `초기화`하기**
...
- `env.py`에 생성해놓은 `config`객체 아래 `main_option`으로서 `sqlalchemy.url` key에 set해준다.
...
- `target_metadata` 객체 **우리의 `Base객체.metadata`를 연결해주고, `entity Model`들을 다 메모리에 import해놓는다.**
...
- run_migration_online에서 타입비교 추가해주기
