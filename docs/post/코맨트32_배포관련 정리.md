...
![image-20230209230506296](https://raw.githubusercontent.com/is2js/screenshots/main/image-20230209230506296.png)



![image-20230209230604493](https://raw.githubusercontent.com/is2js/screenshots/main/image-20230209230604493.png)
...
- 서버는 ORM만 바뀐상태로 push가 온다. 코드는 미리 수정되어있으니 다운grade만 해서 db만 바꾸며 ㄴ된다.





##### [서버전용] 서버에서 revision만들고 관리하기

1. **최초 revision -m "message" 만들고, update head 눌러놓고 서버에 push하기**



```power
alembic revision -m "init entities"                
alembic upgrade head   
```



2. **orm 수정후 revision은 --autogenerate 옵션 추가하여 -m "msg"**



![image-20230209223332653](https://raw.githubusercontent.com/is2js/screenshots/main/image-20230209223332653.png)

```shell
alembic stamp head
INFO  [alembic.runtime.migration] Context impl SQLiteImpl.
INFO  [alembic.runtime.migration] Will assume non-transactional DDL.
INFO  [alembic.runtime.migration] Running stamp_revision afbb75e7ffff -> 1a958ec203ba
...
![image-20230209223423568](https://raw.githubusercontent.com/is2js/screenshots/main/image-20230209223423568.png)
...
##### git pull -> gunicorn restart

```shell
git pull
sudo systemctl restart sqlalchemy
```
